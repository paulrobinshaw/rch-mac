...
The worker MUST echo `protocol_version: 0` in probe responses (since no version has been negotiated yet).
...
The JSON header line MUST be serialized as compact single-line JSON (no unescaped newline characters
within string values). Implementations MUST use standard JSON string escaping (which encodes literal
newlines as `\n`). Pretty-printed or multi-line JSON headers are a protocol violation.
...
- For rejected steps, `job_key` in `summary.json` MUST be `null` (source bundling and key computation
  may not have occurred). Consumers MUST accept `null` for `job_key` when `status=rejected`.
...
- `steps`: ordered array of `{ index, action, job_id, rejected }` where `rejected` is a bool (default `false`)
...
- Binary-framed responses (`fetch`) bypass the standard RPC envelope: the JSON header line IS the complete response
  (it MUST include `protocol_version`, `request_id`, `ok: true`, and the payload fields directly, not nested under `payload`).
...
Aggregation rule (applies to both `exit_code` and `status`):
- If any step has `status=rejected`: run `status` MUST be `rejected`, run `exit_code` MUST be 10.
- Else if any step has `status=cancelled`: run `status` MUST be `cancelled`, run `exit_code` MUST be 80.
- Else if any step has `status=failed`: run `status` MUST be `failed`, run `exit_code` MUST be the first failing step's `exit_code` in run order.
- Else: run `status` MUST be `success`, run `exit_code` MUST be 0.
...
Each entry MUST include a `type` field (`file` or `directory`).
For `file` entries, `sha256` and `size` are computed over the file contents as usual.
For `directory` entries, `sha256` MUST be `null` and `size` MUST be `0`.
